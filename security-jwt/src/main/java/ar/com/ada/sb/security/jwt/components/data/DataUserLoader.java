package ar.com.ada.sb.security.jwt.components.data;
...
public class DataUserLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (appEnv.equals("dev") || appEnv.equals("qa")){
            ...
            User admin = new User()
                    .setId(1L).setUsername("admin").setPassword(passwordEncoder.encode("admin"))
                    .setFirstName("admin").setLastName("admin").setEmail("admin@admin.com")
                    .setEnabled(true)
                    .addAuthority(adminAuthority).addAuthority(managerAuthority)
                    .addAuthority(userAuthority);

            User manager = new User()
                    .setId(2L).setUsername("manager").setPassword(passwordEncoder.encode("manager"))
                    .setFirstName("manager").setLastName("manager").setEmail("manager@manager.com")
                    .setEnabled(true)
                    .addAuthority(managerAuthority)
                    .addAuthority(userAuthority);

            User user = new User()
                    .setId(3L).setUsername("user").setPassword(passwordEncoder.encode("user"))
                    .setFirstName("user").setLastName("user").setEmail("user@user.com")
                    .setEnabled(true)
                    .addAuthority(userAuthority);
